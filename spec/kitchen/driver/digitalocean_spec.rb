...
describe Kitchen::Driver::Cloudstack do
  ...
  before(:each) do
    allow_any_instance_of(described_class).to receive(:instance)
      .and_return(instance)
    ENV["CLOUDSTACK_ACCESS_TOKEN"] = "access_token"
    ENV["CLOUDSTACK_SSH_KEY_IDS"] = "1234"
  end
  ...
  describe "#initialize" do
    context "default options" do
      ...
      it "defaults to Access Token from $CLOUDSTACK_ACCESS_TOKEN" do
        expect(driver[:cloudstack_access_token]).to eq("access_token")
      end
    end

    context "CLOUDSTACK_REGION is tor1" do
      before(:each) do
        allow_any_instance_of(described_class).to receive(:instance)
          .and_return(instance)
        ENV["CLOUDSTACK_REGION"] = "tor1"
      end
      it "defaults to region from CLOUDSTACK_REGION" do
  ...
  describe "#create" do
    ...
    context "username and API key only provided" do
      let(:config) do
        {
          cloudstack_access_token: "access_token",
        }
      end

      it "generates a server name in the absence of one" do
        stub_request(:get, "https://api.cloudstack.com/v2/droplets/1234")
          .to_return(create)
        driver.create(state)
  ...
  describe "#destroy" do
    ...
    context "a live server that needs to be destroyed" do
      it "destroys the server" do
        stub_request(:get, "https://api.cloudstack.com/v2/droplets/12345")
          .to_return(find)
        stub_request(:delete, "https://api.cloudstack.com/v2/droplets/12345")
          .to_return(delete)
